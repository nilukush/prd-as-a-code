...
// One .feature file per PRD: a tagged Scenario per requirement, one neutral
// `*` step per acceptance criterion. Free-text ACs carry no Given/When/Then
// semantics, so the `*` keyword keeps the output valid Gherkin without
// inventing structure the source does not have. Tags carry the stable id and
// priority so downstream tooling can trace scenarios back to requirements.
function buildGherkin(prdDir, outPath) {
  const { meta, reqs } = loadPrd(prdDir);
  requireMeta(meta);

  let text = `# Generated by prdc build gherkin from ${meta.id}. Do not edit; regenerate instead.\n`;
  text += `Feature: ${meta.title}\n`;
  for (const r of reqs) {
    const tags = [`@${r.id}`];
    if (r.priority) tags.push(`@${r.priority}`);
    text += `\n  ${tags.join(' ')}\n`;
    text += `  Scenario: ${r.id}: ${r.i_want || r.title || r.id}\n`;
    for (const ac of r.acceptance_criteria || []) text += `    * ${ac}\n`;
  }
